...
# this script (Porter) stems tokens if the token is all lower-case

# Co-Auhtor: Florian Leitner <florian.leitner@gmail.com>
...
function doublec(s)
{
  if (substr(s,length(s),1) == substr(s,length(s)-1,1) && (substr(s,length(s),1) !~ /[aeiou]/))
  {
    return 1;
  } else {
    return 0;
...
function cons(str,i)
{
  ...
  if (substr(str,i,1) == "y")
  {
    if (substr(str,i-1,1) ~ /[aeiou]/) return 1;
    else return 0
  }
  return 1
}
...
function step1ab(str)
{
  ...
  if (str ~ /eed$/) {
    if (m(substr(str,1,length(str)-3))>0 ) str=substr(str,1,length(str)-1);
  } else {
    trunc=0;
    ...
    if (trunc>0)
    {
      str=substr(str,1,length(str)-trunc) ;
      if (str ~ /(at|bl|iz)$/) str=str"e" ;
      else if (doublec(str)==1 && (str !~ /[lsz]$/)) { str=substr(str,1,length(str)-1); }
...
function step4(str) {
  if ( str ~ /al$/ ) {
    if (m(substr(str,1,length(str)-2)) > 1) str=substr(str,1,length(str)-2)
  } else if ( str ~ /[ae]nce$/ ) {
    if (m(substr(str,1,length(str)-4)) > 1) str=substr(str,1,length(str)-4)
  } else if ( str ~ /(er|ic)$/ ) {
    if (m(substr(str,1,length(str)-2)) > 1 ) str=substr(str,1,length(str)-2)
  } else if ( str ~ /[ai]ble$/ ) {
    if (m(substr(str,1,length(str)-4)) > 1 ) str=substr(str,1,length(str)-4)
  } else if ( str ~ /ant$/ ) {
    if (m(substr(str,1,length(str)-3)) > 1 ) str=substr(str,1,length(str)-3)
  } else if ( str ~ /ement$/) {
    if (m(substr(str,1,length(str)-5)) > 1 ) str=substr(str,1,length(str)-5) 
  } else if ( str ~ /ment$/) {
    if (m(substr(str,1,length(str)-4)) > 1 ) str=substr(str,1,length(str)-4) 
  } else if ( str ~ /ent$/) {
    if (m(substr(str,1,length(str)-3)) > 1 ) str=substr(str,1,length(str)-3) 
    ...
    if (m(substr(str,1,length(str)-2)) > 1 ) str=substr(str,1,length(str)-2) 
  } else if ( str ~ /(ism|ate|iti|ous|ive|ize)$/) {
    if (m(substr(str,1,length(str)-3)) > 1 ) str=substr(str,1,length(str)-3)
  }
  return str
}
...
function step5(str) {
  if (str ~ /e$/ && (m(str)>1 || (m(str)==1 && !cvc(substr(str,1,length(str)-1)))))
